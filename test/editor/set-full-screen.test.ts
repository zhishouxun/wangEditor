...
describe('设置全屏', () => {
    ...
    test('编辑器默认初始化全屏菜单', () => {
        ...
        const fullMenuEl = $(`.${toolbarSelector}`).find('.w-e-icon-fullscreen')

        expect(fullMenuEl.length).toBeGreaterThanOrEqual(1)
    })
    ...
    test('调用 setFullScreen 设置编辑器全屏模式', () => {
        ...
        expect($editorParent.className).toContain('w-e-full-screen-editor')
    ...
    test('调用 setUnFullScreen 取消编辑器全屏模式', () => {
        ...
        expect($editorParent.className).not.toContain('w-e-full-screen-editor')
